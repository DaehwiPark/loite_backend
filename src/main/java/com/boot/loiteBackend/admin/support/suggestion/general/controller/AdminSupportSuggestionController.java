package com.boot.loiteBackend.admin.support.suggestion.general.controller;
...
import com.boot.loiteBackend.admin.support.suggestion.general.service.AdminSupportSuggestionService;
...
public class AdminSupportSuggestionController {
    ...
    private final AdminSupportSuggestionService adminSupportSuggestionService;

    @Operation(summary = "제안 목록 페이징 조회", description = "삭제되지 않은 제안 목록을 페이지 단위로 조회합니다.")
    @GetMapping
    public ResponseEntity<Page<AdminSupportSuggestionDto>> getPagedSuggestions(
            @PageableDefault(page = 0, size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable,
            @RequestParam(required = false) String keyword
    ) {
        Page<AdminSupportSuggestionDto> result = adminSupportSuggestionService.getPagedSuggestions(keyword, pageable);
        return ResponseEntity.ok(result);
    }

    @Operation(summary = "자료 상세 조회", description = "제안 ID를 통해 단일 제안의 상세 내용을 조회합니다. 첨부파일 및 작성자 정보 포함.")
    @GetMapping("/{id}")
    public ResponseEntity<AdminSupportSuggestionDto> getSuggestionById(@PathVariable Long id) {
        AdminSupportSuggestionDto suggestion = adminSupportSuggestionService.getSuggestionById(id);
    ...
    public ResponseEntity<Void> deleteSuggestion(@PathVariable Long id) {
        adminSupportSuggestionService.deleteSuggestion(id);
    ...
    public ResponseEntity<Void> updateReviewAndReplyStatus(
            ...
            @RequestBody AdminSupportSuggestionUpdateDto updateDto) {
        adminSupportSuggestionService.updateReviewStatus(id, updateDto);
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<Resource> downloadFile(@PathVariable("id") Long suggestionFileId) {
        return adminSupportSuggestionService.fileDownload(suggestionFileId);
    }
}
