...
public class AdminHomeHeroUpdateRequestDto {
    ...
    @Schema(description = "좌측 이미지 파일명", example = "hero_left2.jpg")
    @Size(max = 255)
    private String leftImageName;

    @Schema(description = "좌측 이미지 물리 경로", example = "/2025/09/hero_left2.jpg")
    @Size(max = 500)
    private String leftImagePath;

    @Schema(description = "좌측 이미지 크기(byte)", example = "500000")
    private Long leftImageSize;

    @Schema(description = "좌측 이미지 MIME 타입", example = "image/png")
    @Size(max = 100)
    private String leftImageType;

    @Schema(description = "우측 텍스트 색상", example = "#FFFFFF")
    @Pattern(regexp = "^$|^#[0-9A-Fa-f]{6}([0-9A-Fa-f]{2})?$",
            message = "rightTextColor는 #RRGGBB 또는 #RRGGBBAA 형식이어야 합니다.")
    @Size(max = 20)
