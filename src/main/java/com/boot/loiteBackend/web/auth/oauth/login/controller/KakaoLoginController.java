package com.boot.loiteBackend.web.auth.oauth.login.controller;

import com.boot.loiteBackend.global.response.ApiResponse;
import com.boot.loiteBackend.web.auth.oauth.login.service.KakaoLoginService;
...
@RestController
@RequestMapping("/api/public/login/kakao")
@RequiredArgsConstructor
@Slf4j
@Tag(name = "카카오 소셜 로그인", description = "카카오 로그인 & 회원가입 API")
...
//    @GetMapping("/callback")
//    @Operation(summary = "카카오 로그인 콜백", description = "카카오 OAuth 인증 후 콜백을 처리하고 JWT 토큰을 발급합니다.")
...
//        LoginResponseDto loginResult = kakaoLoginService.kakaoLoginCallback(code, response);
//        return ResponseEntity.ok(
