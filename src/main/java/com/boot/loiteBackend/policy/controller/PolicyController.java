package com.boot.loiteBackend.policy.controller;

import com.boot.loiteBackend.policy.dto.PolicyDto;
import com.boot.loiteBackend.policy.service.PolicyService;
...
public class PolicyController {
    ...
    @GetMapping
    public ResponseEntity<Page<PolicyDto>> getPagedPolicy(
            @RequestParam(required = false) String keyword,
            @PageableDefault(page = 0, size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable
    ) {
        Page<PolicyDto> result = policyService.getPagedPolicy(keyword, pageable);
